...
<template>
  <div class="bg h-screen gap-10 flex flex-cols items-center justify-center">
    <div
      v-if="!show"
      class="w-1/2 flex items-center justify-center gap-4 flex-col"
    >
      <el-input
        v-model="password"
        class="w-1/2 h-10"
    ...
    <div v-if="show" class="w-[50vw]">
      <div v-if="step === 1" class="flex grid grid-cols-4 flex-wrap gap-6">
        <div v-for="i in visibleWinners" class="flex flex-col text-white">
          <span class="font-bold text-[32px] leading-[36px]">{{
            i.userId
          }}</span>
        </div>
      </div>
